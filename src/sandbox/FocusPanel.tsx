...
export function FocusPanel() {
  ...
  return (
    <div className="flex-1 flex flex-col bg-surface overflow-hidden">
          ...
          {SOURCE_LOCALES.map((id) => (
            <button
              ...
              className={`text-[10px] font-medium px-1.5 py-0.5 rounded-full border ${
                baseLocale === id ? 'bg-subtle text-ink border-line' : 'border-line text-muted'
              }`}
            >
              {LOCALE_LABEL[id]}
            </button>
        ...
        {searchOpen ? (
          <div className="relative shrink-0">
            ...
            <input
              ...
              className="w-40 text-[11.5px] bg-subtle border border-line rounded-full pl-7 pr-6 py-1.5 outline-none focus:border-imely-primary"
            />
            {query && (
              <button
                onClick={() => setQuery('')}
          ...
          <button
            ...
            title="Search"
            className="shrink-0 text-muted hover:text-ink p-1.5 rounded-full hover:bg-subtle"
          >
            <Search size={14} />
          </button>
        )}
      ...
      <div className="shrink-0 px-6 py-2.5 border-b border-line">
        <div className="flex items-center justify-between text-[11.5px] text-muted mb-1.5">
          <span>
            <span className="font-semibold text-ink">{translatedCount.toLocaleString()}</span> /{' '}
        ...
        <div className="flex items-center gap-5 mt-2.5">
          <div className="flex items-center gap-2 min-w-0">
            <span className="text-[10px] text-muted uppercase tracking-wide truncate max-w-[150px]">
              <span className="font-semibold">{pageUnitLabel}</span>{' '}
              <span className="font-semibold text-muted normal-case">
                {pageIndex >= 0 ? pageSections[pageIndex]?.label : '—'}
              </span>{' '}
              done
            </span>
            <div className="w-16 h-1.5 rounded-full bg-subtle overflow-hidden shrink-0">
              <div
      ...
      <div className="shrink-0 border-b border-line px-6 py-3.5 space-y-3">
        <div className="flex items-center gap-2.5">
          <span
            className="w-[136px] shrink-0 text-[10.5px] text-muted truncate text-right"
            title={prevPageLabel}
          >
            {prevPageLabel && (
              <>
                Previous: <span className="text-muted font-medium">{prevPageLabel}</span>
              </>
            )}
          </span>
          <button
            ...
            className="shrink-0 w-9 h-9 rounded-full flex items-center justify-center text-muted hover:bg-subtle disabled:opacity-30 disabled:hover:bg-transparent"
          >
          ...
          <span className="w-[136px] shrink-0 text-[10.5px] text-muted truncate" title={nextPageLabel}>
            {nextPageLabel && (
              <>
                Next: <span className="text-muted font-medium">{nextPageLabel}</span>
              </>
            )}
          </span>
        </div>
        ...
        {overlaySiblings.length > 0 && (
          <div className="flex items-center gap-2.5">
            <span
              className="w-[136px] shrink-0 text-[10px] text-muted truncate text-right"
              title={prevGroupLabel}
            >
              {prevGroupLabel && (
                <>
                  Previous: <span className="text-muted font-medium">{prevGroupLabel}</span>
                </>
              )}
            </span>
            <button
              ...
              className="shrink-0 w-8 h-8 rounded-full flex items-center justify-center text-muted hover:bg-subtle disabled:opacity-30 disabled:hover:bg-transparent"
            >
              <ChevronLeft size={16} />
            </button>
            <div className="flex-1 min-w-0">
            ...
            <span className="w-[136px] shrink-0 text-[10px] text-muted truncate" title={nextGroupLabel}>
              {nextGroupLabel && (
                <>
                  Next: <span className="text-muted font-medium">{nextGroupLabel}</span>
                </>
              )}
            </span>
          </div>
        ...
        {entry ? (
          <div className="max-w-2xl mx-auto px-8 py-7 min-h-full flex flex-col">
            <div className="flex items-center gap-1.5 mb-7">
              <div className="font-mono text-[11.5px] text-muted">{entry.key}</div>
              <button onClick={copyKey} className="text-faint hover:text-muted">
            ...
            <div className="flex items-center justify-between mb-2">
              ...
              {savedTranslation && (
                <button
                  ...
                  className="flex items-center gap-1 text-[11px] text-muted hover:text-imely-primary"
                >
            ...
            {aiSuggestion ? (
              <button
                ...
                className="mb-2.5 w-full text-left text-[13px] bg-imely-mint/30 hover:bg-imely-mint/50 rounded-xl px-3.5 py-2.5 flex items-start gap-2 text-ink transition-colors"
              >
                <Sparkles size={13} className="text-imely-primary shrink-0 mt-0.5" />
                <span>{aiSuggestion}</span>
              </button>
            ) : (
              <div className="mb-2.5 text-[11.5px] text-faint italic">No AI suggestion yet</div>
            )}
            <textarea
              ref={textareaRef}
              value={draftText}
              ...
              className="w-full flex-1 min-h-[140px] text-[19px] text-ink outline-none resize-none bg-subtle border-2 border-line rounded-2xl px-4 py-3.5 focus:border-imely-primary focus:bg-surface transition-colors placeholder:text-faint"
              rows={5}
            />
          ...
          <div className="h-full flex items-center justify-center px-8 text-center text-[14px] text-muted">
      ...
      <div className="shrink-0 border-t border-line px-6 py-4 flex items-center gap-2.5">
        <button
          onClick={() => goRow(-1)}
          disabled={!canPrevRow}
          title="Previous string (←)"
          className="shrink-0 flex items-center gap-1 text-[13px] font-semibold text-ink px-4 py-3 rounded-full border border-line hover:bg-subtle disabled:opacity-30 disabled:hover:bg-transparent active:scale-[0.97] transition-transform"
        >
