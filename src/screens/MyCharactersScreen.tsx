    ...
    <div className="h-full flex flex-col bg-surface relative">
      ...
      <div className="flex-1 overflow-y-auto pb-20">
        {MOCK_FEED_CHARACTERS.map((c) => (
          <div key={c.id} className="flex gap-3 px-4 py-3.5 border-b border-line last:border-0">
            <button onClick={() => startChat(c)} className="flex-1 flex gap-3 text-left active:opacity-70 transition-opacity">
              ...
              <div className="min-w-0">
                <div className="font-bold text-[15px] text-ink">{ph(c.name, baseLocale)}</div>
                <div className="text-[12.5px] text-muted line-clamp-2 mt-0.5">{ph(c.biography, baseLocale)}</div>
              </div>
            </button>
            <button
              onClick={() => setMenuFor(c)}
              className="shrink-0 text-muted active:scale-90 transition-transform self-start"
            >
      ...
      <button
        ...
        className="absolute bottom-5 left-1/2 -translate-x-1/2 flex items-center gap-1.5 bg-surface border border-line rounded-full pl-3.5 pr-4 py-2.5 shadow-lg active:scale-95 transition-transform"
      >
          ...
          <div className="absolute inset-0 z-10">
            ...
            <div className="absolute bottom-0 left-0 right-0 bg-surface rounded-t-3xl">
              <div className="flex items-center justify-between px-4 py-4 border-b border-line">
                ...
                <button onClick={() => setMenuFor(null)} className="text-muted active:scale-90 transition-transform">
                  <X size={18} />
                </button>
              </div>
              <button
        ...
        <ZoneScope zone="delete_confirm">
          <div className="absolute inset-0 z-20">
            ...
            <div className="absolute left-4 right-4 top-1/2 -translate-y-1/2 bg-surface rounded-2xl p-5">
              ...
              <div className="mt-1 text-[13px] text-muted">{ph(deleteConfirmFor.name, baseLocale)}?</div>
