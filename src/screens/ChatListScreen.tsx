    ...
    <div className="pb-6">
      {MOCK_FEED_CHARACTERS.map((c, i) => (
        <button
          ...
          className={`w-full flex items-center gap-3 px-4 py-3 border-b border-line last:border-0 text-left active:bg-subtle transition-colors ${
        ...
        >
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="text-[13px] text-muted truncate">{ph(c.firstMessage, baseLocale).replace(/\n/g, ' ')}</div>
          </div>
          <div className="flex flex-col items-end gap-1.5 shrink-0">
            <div className="text-[11px] text-muted">{PLACEHOLDER_DATES[i] ?? PLACEHOLDER_DATES[0]}</div>
            {i === 0 ? (
